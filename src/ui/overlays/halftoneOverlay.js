...
export function hitTestHalftone(e) {
    ...
    if (p.halftoneFadeEnabled) {
        ...
        const shape = p.halftoneFadeShape ?? 'ellipse';
        const angle = (p.halftoneFadeAngle ?? 0) * Math.PI / 180;
        const a = (p.halftoneFadeW / 100) * W / 2;
        const b = (p.halftoneFadeH / 100) * H / 2;
...
export function onDragHalftone(e, inst, rect) {
    ...
    if (handle === 'fadeCenter') {
        setInstanceParam(state.instId, 'halftoneFadeX', Math.round(Math.max(-50, Math.min(50,  (mx / W - 0.5) * 100))));
        setInstanceParam(state.instId, 'halftoneFadeY', Math.round(Math.max(-50, Math.min(50, -(my / H - 0.5) * 100))));
    } else if (handle === 'fadeEdgeW') {
        setInstanceParam(state.instId, 'halftoneFadeW', Math.round(Math.max(1, Math.min(200, Math.abs(mx - fcx) / (W / 2) * 100))));
    } else if (handle === 'fadeEdgeH') {
        setInstanceParam(state.instId, 'halftoneFadeH', Math.round(Math.max(1, Math.min(200, Math.abs(my - fcy) / (H / 2) * 100))));
    } else if (handle === 'fadeRot') {
        let deg = Math.atan2(my - fcy, mx - fcx) * 180 / Math.PI + 90;
        if (deg > 180)  deg -= 360;
        if (deg < -180) deg += 360;
        setInstanceParam(state.instId, 'halftoneFadeAngle', Math.round(deg));
    }
}
