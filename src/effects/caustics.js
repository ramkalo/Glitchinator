...
export const causticsEffect = {
    ...
    paramKeys: ['causticsNumber', 'causticsSize', 'causticsChop', 'causticsSmoothing',
                'causticsCrossAngle', 'causticsLineDeviation',
                'causticsSharpness', 'causticsDiffusion', 'causticsBrightness', 'causticsContrast',
                'causticsDispersion', 'causticsSeed',
    ...
    overlays: {},
    uiGroups: [
        { keys: ['causticsNumber', 'causticsSize', 'causticsChop', 'causticsSmoothing',
                 'causticsCrossAngle', 'causticsLineDeviation',
                 'causticsSharpness', 'causticsDiffusion', 'causticsBrightness', 'causticsContrast',
                 'causticsDispersion', 'causticsColor', 'causticsShuffleBtn'] },
        blend.uiGroup,
        fade.uiGroup,
    ],
    params: {
        ...
        causticsSize:       { default: 45, min: 0, max: 100, label: 'Size' },
        causticsChop:       { default: 45, min: 0, max: 100, label: 'Chop' },
        causticsSmoothing:  { default: 60, min: 0, max: 100, label: 'Smoothness' },
...
uniform float causticsSmoothing;
...
// Value noise whose interpolation blends linear (sm=0 → sharp corners) to
// smoothstep (sm=1 → smooth curves). Used only for the line wiggle trajectory,
// so Smoothness controls jagged vs. curved lines without touching brightness.
float causticsVN2m(vec2 p, float sm) {
    vec2 i = floor(p), f = fract(p);
    vec2 u = mix(f, f * f * (3.0 - 2.0 * f), sm);
    float a = causticsHash2(i);
    float b = causticsHash2(i + vec2(1.0, 0.0));
    float c = causticsHash2(i + vec2(0.0, 1.0));
    float d = causticsHash2(i + vec2(1.0, 1.0));
...
vec3 causticsFamily(vec2 base, float baseAng, float fN, float span,
                    ...
                    float devRad, float wampl, float wfreq, float sm, float sd) {
    vec3 val = vec3(0.0);
    for (int i = 0; i < 32; i++) {
        if (float(i) >= fN) break;
        float fi  = float(i);
        ...
        float wig = ((causticsVN2m(vec2(al * wfreq,       fi * 4.3 + sd),       sm) * 2.0 - 1.0)
                  +  (causticsVN2m(vec2(al * wfreq * 2.6, fi * 4.3 + sd + 5.0), sm) * 2.0 - 1.0) * 0.5) * wampl;
...
void main() {
    ...
    if (causticsBrightness > 0.0) {
        ...
        float span    = 2.0;
        float unit    = mix(0.03, 0.35, causticsSize / 100.0);                  // line size, independent of fN
        float diff    = causticsDiffusion / 100.0;
        float coreW   = unit * mix(0.30, 0.04, causticsSharpness / 100.0);      // line width
        float glowW   = unit * mix(0.18, 0.90, diff);                           // glow spread
        ...
        float wampl   = unit * mix(0.08, 1.5, causticsChop / 100.0);            // per-line wiggle amount
        float wsm     = causticsSmoothing / 100.0;                              // wiggle corner shape
        float wfreq   = 5.5;
        float ixAng   = radians(mix(25.0, 155.0, causticsCrossAngle / 100.0));  // cross angle

        vec3 valA = causticsFamily(base, 0.0,   fN, span, coreW, glowW, diff, disp, devRad, wampl, wfreq, wsm, causticsSeed);
        vec3 valB = causticsFamily(base, ixAng, fN, span, coreW, glowW, diff, disp, devRad, wampl, wfreq, wsm, causticsSeed + 91.0);
